...
import 'package:balzanewsweb/widgets/date_picker/date_picker_constants.dart';
import 'package:balzanewsweb/widgets/date_picker/date_picker_i18n.dart';
import 'package:balzanewsweb/widgets/date_picker/date_picker_theme.dart';
import 'package:balzanewsweb/widgets/date_picker/date_time_formatter.dart';
...
class _DateTimePickerWidgetState extends State<CustomDatePicker> {
  ...
  void _onSelectedChange() {
    if (widget.onChange != null) {
      ...
      if (dateTime.month != _currMonth) {
        dateTime = dateTime.copyWith(month: dateTime.month, day: 0);
      }
      if (dateTime.isBefore(widget.minDateTime ?? DateTime.now())) {
        dateTime = widget.minDateTime ?? DateTime.now();
      }
  ...
  Widget _renderDatePickerItemComponent(int value, String format) {
    int? selectedValue;
    if (format.contains('y')) {
      selectedValue = _currYear;
    } else if (format.contains('M')) {
      selectedValue = _currMonth;
    } else if (format.contains('d')) {
      selectedValue = _currDay;
    } else if (format.contains('H')) {
      selectedValue = _currHour;
    } else if (format.contains('m')) {
      selectedValue = _currMinute;
    } else if (format.contains('s')) {
      selectedValue = _currSecond;
    }
    ...
    return Container(
      ...
      child: Text(
        ...
        style: widget.pickerTheme.itemTextStyle.copyWith(
            fontFamily: isSelected ? AppFonts.bold : AppFonts.medium,
            color: Theme.of(context).colorScheme.surfaceDim,
            fontWeight: isSelected ? FontWeight.bold : FontWeight.w500,
            fontSize: isSelected ? 18.fs : 16.fs),
      ),
    );
  }
